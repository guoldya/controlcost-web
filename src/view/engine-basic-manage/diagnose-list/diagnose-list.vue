...
<template>
  <div>
    <Card dis-hover>
      ...
      <Row class="margin-top-10">
        <div id="diagnose-list"></div>
      </Row>
      <Row class="margin-top-10">
        <div id="diagnose-col"></div>
      </Row>
    </Card>
  </div>
</template>
...
<script>
  import Vue from 'vue';
  let diagnose_list_url = 'sys/coreBdDx/selectByPage';
  let diagnose_col_url = 'sys/bdDx/selectByPage';
  let med_sync_compare_url='sys/coreBdDx/updateOfSync ';
  ...
  export default {
    data(){
      return {
        ...
        medDiagnoseListData: new this.customstore({
          load: (loadOptions) => {
            return this.CommDs.loadPage(loadOptions, diagnose_list_url, this.isLoadData, {}, this.medDiagnoseList);
          }
        }),
        medDiagnoseColData:new this.customstore({
          load: (loadOptions) => {
            if(loadOptions.searchData==null)
            {
              ...
              return this.CommDs.loadPage(loadOptions, diagnose_col_url, this.isLoadData, {}, this.medDiagnoseCol)
            }
          },
        }),
    ...
    mounted() {
      ...
      this.medDiagnoseList = $('#diagnose-list').dxDataGrid({
        showBorders: true,
        showRowLines: true,
        ...
        dataSource: {
          store:this.medDiagnoseListData,
        },
        ...
        onSelectionChanged:(e)=> {
          if (e.selectedRowsData[0]) {
            var ds = this.medDiagnoseCol.getDataSource();
            this.insCodeParamas=e.selectedRowsData[0].insCode;
      ...
      this.medDiagnoseCol = $('#diagnose-col').dxDataGrid({
        showBorders: true,
        showRowLines: true,
        columnAutoWidth: true,
        //wordWrapEnabled: true,
    ...
    methods: {
      search: function () {
        var ds = this.medDiagnoseList.getDataSource();
        ds.loadOptions().searchData = this.searchFormData;
        ds.reload();
        this.medDiagnoseList.selectRows();
        var dx = this.medDiagnoseCol.getDataSource();
      ...
      syncCompare: function (data) {
        let params={};
        params.insCode=this.insCodeParamas;
        this.$put(med_sync_compare_url).then(res => {
          if (res.code == '200') {
            this.success('成功!', () => {
              this.medDiagnoseList.refresh();
            });
