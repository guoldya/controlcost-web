...
<template>
  <div>
    <Card dis-hover>
      ...
      <Row class="margin-top-10">
        <div id="med-consume-list"></div>
      </Row>
      <Row class="margin-top-10">
        <div id="med-consume-col"></div>
      </Row>
    </Card>
  </div>
</template>
...
<script>
  ...
  let med_consume_col_url='sys/bdInsMc/selectByPage';
  let med_sync_compare_url='sys/coreBdInsMc/updateOfSync';
  ...
  export default {
    data(){
      return {
        ...
        medConsumeListData: new this.customstore({
          load: (loadOptions) => {
            return this.CommDs.loadPage(loadOptions, med_consume_list_url, this.isLoadData, {}, this.medConsumeList);
          }
        }),
        medConsumeColData:new this.customstore({
          load: (loadOptions) => {
            if(loadOptions.searchData==null)
            {
              ...
              return this.CommDs.loadPage(loadOptions, med_consume_col_url, this.isLoadData, {}, this.medConsumeCol)
            }
          },
        }),
    ...
    mounted() {
      ...
      this.medConsumeList = $('#med-consume-list').dxDataGrid({
        showBorders: true,
        showRowLines: true,
        ...
        onSelectionChanged:(e)=> {
          if (e.selectedRowsData[0]){
            var ds = this.medConsumeCol.getDataSource();
      ...
      this.medConsumeCol = $('#med-consume-col').dxDataGrid({
        showBorders: true,
        showRowLines: true,
        columnAutoWidth: true,
        //wordWrapEnabled: true,
    ...
    methods: {
      search: function () {
        var ds = this.medConsumeList.getDataSource();
        ds.loadOptions().searchData = this.searchFormData;
        ds.reload();
        this.medConsumeList.selectRows();
        var dx = this.medConsumeCol.getDataSource();
