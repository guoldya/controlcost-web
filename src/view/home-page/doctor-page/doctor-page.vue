<template>
  <div>
    <Card dis-hover>
      <Row class="RowOne">
        <ul class="ulList">
          <li><DatePicker class="dataPicker" type="daterange" placement="bottom-end"
                          @on-change="handleChange"
                          format="yyyy-MM-dd"
                          placeholder="请选择时间段"></DatePicker></li>
    ...
    <Row class="RowFour">
      <Col span="12" >
        <Card dis-hover class="cardScalePel">
          <p slot="title">项目类别占比</p>
          <v-chart class="chartPel" :forceFit="true" :height="itemType.height" :data="itemType.data" :scale="itemType.scalePel" :onClick="clickChange">
  ...
  export default {
    ...
    data() {
      return {
        ...
        inforCardData: [
          { title: '个人提醒总量/次', text: '审', count: 0, color: '#9A66E4',detailNumber: 1 },
          { title: '个人提醒总量/次', text: '警', count: 0, color: '#ff9900',detailNumber: 2 },
          { title: '个人提醒总量/次', text: '禁', count: 0, color: '#ed3f14',detailNumber: 3 },
        ...
        //     // axisTick:{lineStyle:{color:'#8c8c8c'}},
        //
        //   },
        //   yAxis: {
        //     type: 'value',
    ...
    methods: {
      ...
      onLood: function () {
        ...
        let params = {};
        let foldList = [];
        let color = ['#5db1ff', '#36cbcb', '#4ecb73', '#fbd437', '#666666', '#ed3f14'];
        _this.listData = [];
        ...
        this.$put(core_rule_count_ratio_url, params).then(res => {
          if (res.code == '200') {
            for (let i = 0; i < res.rows.length; i++) {
              ...
              _this.listData.push({
                itemBzCode: res.rows[i].itemBzCode, itemBzCodeStr: res.rows[i].itemBzCodeStr,
        ...
        this.$put(core_rule_fold_list_url, params).then(res => {
          if (res.code == '200') {
            ...
            if (params.index == 1) {
              ...
              for (let key in this.foldType.sourceData) {
                this.chartListData.push(this.foldType.sourceData[key].time);
                this.chartLineOne.push(this.foldType.sourceData[key].审核);
                this.chartLineTwo.push(this.foldType.sourceData[key].警告);
                this.chartLineThree.push(this.foldType.sourceData[key].禁止);
              }
              // this.chart.setOption({
              //   series: [
              //     {data: this.chartLineOne},
              //     {data: this.chartLineTwo},
              //     {data: this.chartLineThree}
              //   ],
              //   xAxis: {
