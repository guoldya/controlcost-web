...
<template>
  <div>
    ...
    <Modal v-model="modal.visible" :mask-closable="false" width="650" title="编辑数据">
      <p slot="header">
        <span v-if="modal.isNew">新增药品</span>
    ...
    <Modal v-model="modalDrug.visible" :mask-closable="false" width="650" title="编辑数据">
      ...
      <div slot="footer">
        ...
        <Button type="primary" :loading="modal.modal_loading" @click="DrugSubmit">保存</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  ...
  let special_query_list_url = 'sys/coreBdDiseasesDrug/selectByDrugList';

  let special_insert_batch_url = 'sys/coreBdDiseasesDrug/insertByBatch';

  let special_update_batch_url = 'sys/coreBdDiseasesDrug/updateByBatch';

  let special_all_drugs_url = 'sys/coreBdInsMed/selectList';
  ...
  export default {
      ...
      return {
        ...
        specialGridData: new this.customstore({
          load: (loadOptions) =>{
            ...
            return this.CommDs.loadPage(loadOptions, special_disease_url, this.isLoadData, {}, this.specialGrid);
          }
        }),
        drugListGrid: {},
        drugListGridData: new this.customstore({
          load: (loadOptions) =>{
            if (loadOptions.searchData == null){
              ...
              // return this.CommDs.loadPage(loadOptions, special_query_list_url, this.isLoadData, {}, this.drugListGrid);
              return this.CommDs.loadList(special_query_list_url,loadOptions.searchData);
            }
          }
        }),
        allDrugListGrid:{},
        allDrugListGridData: new this.customstore({
          load: (loadOptions) =>{
              // return this.CommDs.loadPage(loadOptions, special_query_list_url, this.isLoadData, {}, this.drugListGrid);
              return this.CommDs.loadList(special_all_drugs_url,loadOptions.searchData);
          }
          // load: (e) =>{
          //   return this.CommDs.loadList( special_all_drugs_url);
          // }
        }),
      }
    },
    mounted() {
        ...
        onOptionChanged:function(e){
          if(e.name == "searchPanel"){
            if(typeof (e.value) == 'string'){
              _this.specialGrid.getDataSource().loadOptions().searchData = {keyword:e.value,dtype: 1};
      ...
      this.btnPart.items = [
        ...
        {id: 2, text: '批量管理', type: 'primary', enabled: true, click: this.batch},
      ];
    ...
    methods: {
      ...
      search: function(){
        if(this.specialGrid.getSelectedRowKeys().length <1)
        {
          this.warn("请选择事件原型");
          return;
        }
      ...
      formSubmit:function () {
        ...
        if (this.allDrugs.length == 0){
          ...
          this.$post(special_insert_batch_url, this.editFormData).then(res => {
            if (res.code == '200') {
              this.success('保存成功!', () => {
                this.modal.modal_loading = false;
                this.modal.visible = false;
      ...
      DrugSubmit: function () {
        ...
        this.$post(special_update_batch_url, this.editDrugFormData).then(res => {
          if (res.code == '200') {
            this.success('保存成功!', () => {
              this.modalDrug.modal_loading = false;
              this.modalDrug.visible = false;
