  ...
  <div>
    <Row>
      ...
      <Modal v-model="modal.visible" :mask-closable="false" width="650" title="编辑数据">
        <p slot="header">
          <span v-if="modal.isNew">添加慢病</span>
          <span v-else="modal.isNew">编辑慢病</span>
        </p>
        <div id="editForm"></div>
        <div slot="footer">
...
<script>
  ...
  let slow_disease_insert = 'sys/coreBdDiseases/updateOrinsert';
  let slow_disease_updateStatus = 'sys/coreBdDiseases/updateStatus';
  ...
  export default {
    ...
    mounted() {
      let _this = this;
      _this.searchForm = $('#searchForm').dxForm({
        colCount: 4,
      ...
      this.btnPart.items = [
        {id: 1, text: '添加', type: 'primary', enabled: true, click: this.addSlow},
      ];
    ...
    methods: {
      ...
      search: function () {
        var ds = this.slowGrid.getDataSource();
      ...
      updateStatus: function (data) {
        this.$post(slow_disease_updateStatus, data).then(res => {
          if (res.code == '200') {
            this.success('成功!', () => {
              this.slowGrid.refresh();
            });
      ...
      formSubmit: function () {
        ...
        this.$post(slow_disease_insert, this.editFormData).then(res => {
          if (res.code == '200') {
            this.success('保存成功!', () => {
              this.modal.modal_loading = false;
              this.modal.visible = false;
